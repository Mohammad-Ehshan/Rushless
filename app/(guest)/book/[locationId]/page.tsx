...
import { ParkingLocation } from '@/schemas/parkingLocations'
    ...
    return (
        <div className='h-full'>
            <main className="sm:-mt-16 sm:container flex flex-col items-center">
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className='bg-white w-[400px] sm:w-[700px] border p-4 shadow flex flex-col pt-12 pb-6 space-y-4 z-10' >
                        <div>
                        ...
                        <div className="flex flex-col bg-slate-100 p-4 gap-y-2 rounded">
                            <div className="flex justify-between text-sm font-bold">
                                <p>Hourly rate</p>
                                <p>{location ? formatAmountForDisplay(location.price.hourly, 'Rs') : '...'}</p>
                            </div>
                            <div className="flex justify-between text-sm font-bold">
                                <p>{diffInHours} Hours</p>
                                <p>{location ? formatAmountForDisplay(diffInHours * location.price.hourly, 'Rs') : '...'}</p>
                            </div>
                        </div>
                        ...
                        <FormField
                            ...
                            render={({ field}) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input className='uppercase' placeholder='Dl VH 5429' {...field} />
                                    </FormControl>
                                    <FormDescription>
                                    Make sure your vehicle's registration number matches the one you parked to avoid fines or towing as per Indian traffic rules.
                                    </FormDescription>
                                </FormItem>
                            )}
                        />  
